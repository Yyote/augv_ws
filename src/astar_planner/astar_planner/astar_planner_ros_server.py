...
class GlobalPlanner(Node):
	...
	#self.filter_traj_threshold = rclpy.get_param('~filter_traj_threshold', 0.5)
        self.filter_traj_threshold = 0.5
        ...
        self.marker_path_pub = self.create_publisher(Marker, "/astar/viz/global_path", 10)
        # TODO: сделать поинтклауд вместо маркеров, должно разгрузить
        self.marker_obs_pub = self.create_publisher(MarkerArray, "/astar/viz/obstacles", 10)
    ...
    def display_path(self, trajectory: list):
        ...
        for i in range(len(trajectory)):
            waypoint = Point()
            # waypoint.header.stamp = rclpy.get_time()
