...
class OmniRegulator : public GroundRegulator
{
    public:
    OmniRegulator()
    {
        ...
        goal_pub = this->create_publisher<augv_navigation_msgs::msg::Position>(this->robot_ns + "/goal", 10);
    }
    ...
    geometry_msgs::msg::Twist logic(float yaw_singal, float x_signal, float y_signal, float z_signal) override
    {
            ...
            twist.linear.x = x_signal + field_vel.twist.linear.x / 5;
            // twist.linear.y = y_signal + field_vel.twist.linear.y / 1;
            twist.linear.y = y_signal + field_vel.twist.linear.y / 5;
            // twist.linear.x = 0;
...
int main (int argc, char * argv[])
{
    ...
    rclcpp::spin(std::make_shared<OmniRegulator>());
    return 0;
}